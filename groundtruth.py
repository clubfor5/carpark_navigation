...
import time
def getN(posA, posB,samplePeriod):
    tsA = posA['ts']
    tsB = posB['ts']
    N =int((tsB - tsA) / (samplePeriod*1000))
    return N 

def generateGT(rawPosTable,samplePeriod):
    gtPos = []
    for  i in range(len(rawPosTable)-1):
        N = getN(rawPosTable[i], rawPosTable[i+1],samplePeriod)
