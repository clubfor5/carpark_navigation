...
class Particle(object):
    ...
    def advance_by(self, speed, checker=None, noisy=True):
        ...
        r = math.radians(h)
        dx = math.sin(r) *  speed * random.uniform(-0.3,1)
        dy = math.cos(r) * speed* random.uniform(-0.3,1)
...
if __name__ == '__main__':
    ...
    particleNum = 250
    for i in range(6):
        rmsParticle,period_pf = pf_process(particleNum)
        rmsError.append(rmsParticle)
        computationTime.append(period_pf)
        particleNum *= 2
        print(rmsParticle,computationTime)
    plt.plot(computationTime, rmsError)
    plt.show()
